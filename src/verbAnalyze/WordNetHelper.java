...
public class WordNetHelper {
	...
	public static double[] getLexFileIdsFraction(String s) throws JWNLException {
		IndexWord word = getWord(POS.VERB, s);
		...
		int sum = 0;
		for (int i = 0; i < senses.length; i++) {
			int idx = (int) (senses[i].getLexFileId()) - 29;
			ret[idx] += wordnet.getUsageCount(senses[i].getOffset(), s);
			sum += ret[idx];
		}
		if (sum == 0) {
			sum = 1;
	...
	public static void showRelatedTree(IndexWord word, int depth,
	...
	public static String getKey(String s, long l) {
		return wordnet.getSenseKey(l, s);
	}
